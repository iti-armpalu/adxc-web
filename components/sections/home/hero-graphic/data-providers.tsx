...
export function DataProviders({ activeQuestion, scale = 1 }: DataProvidersProps) {
    ...
    return (
        <div className="relative shrink-0" style={{ width: WIDTH, height: HEIGHT }} aria-label="Data providers">
            <svg key={activeQuestion} className="absolute inset-0 overflow-visible pointer-events-none" width={WIDTH} height={HEIGHT}>
            ...
            {PROVIDER_DOTS.map((p, i) => {
                ...
                return (
                    ...
                    >
                        {p.logo ? (
                            <Image src={p.logo} alt={p.label ?? ""} width={p.size} height={p.size}
