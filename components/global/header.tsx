...
type Props = { latestPosts?: BlogPostPreview[] }
...
type MobileProps = {
    ...
    setMobileExpanded: (v: ((prev: string | null) => string | null)) => void
    pathname: string
}
...
export function Header({ latestPosts = [] }: Props) {
    ...
    useEffect(() => {
        const handler = (e: KeyboardEvent) => {
            if (e.key === "Escape") { setOpenPanel(null); setMobileOpen(false) }
        }
    ...
    return (
        ...
        >
            <div className={cn(
                ...
                <DesktopNav pathname={pathname} openPanel={openPanel} togglePanel={togglePanel} />
                <DesktopCTAs />
                <MobileSheet
            ...
            {/* Rendered via portal to escape the header's backdrop-blur stacking context */}
            {openPanel && typeof document !== "undefined" && createPortal(
                <div
                    className="fixed inset-x-0 bottom-0 bg-black/40"
                    style={{ top: headerRef.current?.offsetHeight ?? 80, zIndex: 49 }}
                    onClick={() => setOpenPanel(null)}
                />,
                document.body
            )}

            <MegaPanels openPanel={openPanel} latestPosts={latestPosts} headerHeight={headerHeight} />
...
function DesktopNav({ pathname, openPanel, togglePanel }: NavProps) {
    ...
    return (
        <nav className="hidden lg:flex items-center gap-1 flex-1 justify-center">
            {siteConfig.nav.map((group) => {
                ...
                if (!hasPanel && !group.href) return null
                ...
                if (!hasPanel && group.href) {
                    return (
                        <Link
                            ...
                            href={group.href}
                            onClick={() => {
                                trackNavLinkClicked(group.label, group.href!)
                            }}
                            className={cn(
...
function DesktopCTAs() {
    return (
        <div className="hidden lg:flex items-center gap-2 shrink-0">
            ...
            >
                <Link href={siteConfig.secondaryCta.href} target="_blank" rel="noopener noreferrer">
...
function MegaPanels({ openPanel, latestPosts, headerHeight }: {
    openPanel: string | null
            ...
            {siteConfig.nav.map((group) => {
                ...
                return (
                    <div
                        key={group.label}
                        className={cn(
                            "hidden lg:block fixed inset-x-0 max-w-7xl mx-auto px-6 pt-6 z-50",
...
function ResourcesPanel({ posts }: { posts: BlogPostPreview[] }) {
    return (
        <div className="space-y-1">
            ...
            {posts.slice(0, 3).map((post) => (
                ...
                >
                    <div className="space-y-0.5">
                        <p className="text-sm font-medium text-foreground leading-snug line-clamp-2">{post.title}</p>
            ...
            <Link href="/blog" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors pt-2">
                View all posts
